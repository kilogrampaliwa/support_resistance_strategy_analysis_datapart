import pandas as pd

def prepareFileForexter(input_path: str, output_path: str, interval: str = "15min"):
    print(f"=== prepareFileForexter ===\nInput: {input_path}\nOutput: {output_path}\nInterval: {interval}")

    df = pd.read_csv(
        input_path,
        names=["TICKER", "DATE", "TIME", "OPEN", "HIGH", "LOW", "CLOSE", "VOL"],
        header=0
    )

    def normalize_time_column(time_series: pd.Series) -> pd.Series:
        time_fixed = time_series.astype(str).str.zfill(6)
        invalid_times = time_fixed[~time_fixed.str.match(r'^\d{6}$')]
        if not invalid_times.empty:
            raise ValueError(f"Wrong TIME values after normalization:\n{invalid_times}")

        hours = time_fixed.str.slice(0, 2).astype(int)
        minutes = time_fixed.str.slice(2, 4).astype(int)
        seconds = time_fixed.str.slice(4, 6).astype(int)

        if ((hours > 23) | (minutes > 59) | (seconds > 59)).any():
            raise ValueError("TIME contains unrealistic values after normalization.")

        return time_fixed

    df["TIME"] = normalize_time_column(df["TIME"])

    df["DATETIME"] = pd.to_datetime(
        df["DATE"].astype(str) + df["TIME"],
        format="%Y%m%d%H%M%S"
    )
    df = df.set_index("DATETIME")

    df_resampled = df.resample(interval).agg({
        "DATE": "first",
        "TIME": "first",
        "OPEN": "first",
        "HIGH": "max",
        "LOW": "min",
        "CLOSE": "last"
    }).dropna()

    df_resampled["DATE"] = df_resampled.index.strftime("%Y%m%d")
    df_resampled["TIME"] = df_resampled.index.strftime("%H%M%S")

    df_resampled["WEIGHTED_AVG"] = (
        ((df_resampled["HIGH"] + df_resampled["LOW"]) / 2 + 
         2 * ((df_resampled["OPEN"] + df_resampled["CLOSE"]) / 2)) / 3
    )

    df_resampled = df_resampled[["DATE", "TIME", "OPEN", "HIGH", "LOW", "CLOSE", "WEIGHTED_AVG"]]

    df_resampled.to_csv(output_path, header=False, index=False, float_format="%.5f")
    print(f"No candles: {len(df_resampled)} || Saved in:\n{output_path}")
